...
class Ne2wSpider(scrapy.Spider):
    name = 'ne2w'
    start_urls = ['https://www.arabam.com/ikinci-el/otomobil/']

    # model yil km renk fiyat il
    def parse(self, response):
        BMW = response.xpath('//*[@id="js-hook-for-listen-scroll-cta"]/div[1]/ul[2]/li/ul/li[6]/a/@href').get()
        HONDA = response.xpath('//*[@id="js-hook-for-listen-scroll-cta"]/div[1]/ul[2]/li/ul/li[21]/a/@href').get()
        CITROEN = response.xpath('//*[@id="js-hook-for-listen-scroll-cta"]/div[1]/ul[2]/li/ul/li[11]/a/@href').get()

        for auto in [BMW, HONDA, CITROEN]:
            url = response.urljoin(auto)
            yield scrapy.Request(url, callback=self.parse_page)

    def parse_page(self, response):
        # arabalar = response.xpath('//*[@id="main-listing"]/tbody')
        arabalar = response.css('tbody tr')
        for i in arabalar:  # .css('td a ::text').getall()
